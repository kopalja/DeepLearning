...
class Network:
    def __init__(self, args, num_words, num_tags, num_chars):
        ...
        cle_embed = tf.keras.layers.Embedding(num_chars, args.cle_dim, mask_zero=True)(charseqs)
        gru_cell = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(args.cle_dim))(cle_embed)
        ...
        word_embed = tf.keras.layers.Embedding(num_words, args.we_dim, mask_zero=True)(word_ids)
        concat = tf.keras.layers.Concatenate()([word_embed, gather])
        ...
        bidir_cell = tf.keras.layers.Bidirectional(rnn_cell)(concat)

        # TODO(we): Add a softmax classification layer into `num_tags` classes, storing
        # the outputs in `predictions`.
        predictions = tf.keras.layers.Dense(num_tags, activation=tf.keras.activations.softmax)(bidir_cell)
